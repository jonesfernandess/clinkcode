...
export class CommandHandler {
  ...
  async handleModel(ctx: Context): Promise<void> {
    ...
    const messageText = ctx.message && 'text' in ctx.message ? ctx.message.text : '';
    const modelArg = messageText.replace(/^\/model(?:@\w+)?/i, '').trim().toLowerCase();
    ...
    if (modelArg) {
      ...
      if (matchedModel) {
        ...
        await ctx.reply(this.formatter.formatError(`Unknown model: "${modelArg}". Use /agentconfig to see available options.`), { parse_mode: 'MarkdownV2' });
        return;
      }
    }
