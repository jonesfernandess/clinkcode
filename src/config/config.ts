...
import { AgentModel, AgentProvider } from '../models/types';
import { ensureAgentConfigFile } from '../services/agent-config-store';
...
export function loadConfig(): Config {
  ...
  const agentProvider = agentDefaults.provider;
...
export function validateConfig(config: Config): void {
  ...
  if (config.agent.provider !== 'claude' && config.agent.provider !== 'codex') {
    throw new Error('Agent provider must be either "claude" or "codex"');
  }
